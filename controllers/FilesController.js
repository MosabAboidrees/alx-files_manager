...
import { getUserFromXToken } from '../utils/authUtils';
...
// Define root folder ID
const ROOT_FOLDER_ID = 0;

// Define default root folder name
...
// Task 9. Image Thumbnails (Define the thumbnail generation worker)
// Define the FilesController class
export default class FilesController {
  // Task 5. First file (Upload file)
  /**
   * Uploads a file.
  ...
  static async postUpload(req, res) {
    // Extract user and file details from request
    const { user } = req;
    ...
    // Define user ID and base directory
    const userId = user._id.toString();
    const baseDir = `${process.env.FOLDER_PATH || ''}`.trim().length > 0
      ? process.env.FOLDER_PATH.trim()
      : joinPath(tmpdir(), DEFAULT_ROOT_FOLDER);

    // Define new file object
    const newFile = {
    ...
    // If the file is not a folder, write the file to the local path
  ...
  // Task 6. Get and list file (File details)
  /**
  ...
  static async getShow(req, res) {
    // Extract user and file ID from request
    const { user } = req;
    ...
    // If the file is not found, respond with an error
    if (!file) {
  ...
  static async getIndex(req, res) {
    // Extract user and query parameters from request
    const { user } = req;
  ...
  // Task 7. Publish and unpublish files
  /**
   * Publishes a file.
   * @param {Request} req The Express request object.
   * @param {Response} res The Express response object.
  ...
  static async getFile(req, res) {
    ...
    // If the file is not found or not accessible, respond with an error
    if (!file || (!file.isPublic && (file.userId.toString() !== userId))) {
      res.status(404).json({ error: 'Not found' });
      return;
    }

    // If the file is a folder, respond with an error
    if (file.type === VALID_FILE_TYPES.folder) {
